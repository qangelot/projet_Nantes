...
def oversample_indices_clf(inds, y):
    """
    Balance number of samples for all classes (if classification)
...
def run_models(models_n_params, x, y, isClassification,
             test_size = 0.2, n_splits = 5, random_state=None, Oversample=True,
             scoring=None,
    ...
    for i, (clf_class_, parameters) in enumerate(models_n_params):
        try:
            ...
            clf_search = GridSearchCV(clf_class_(), parameters, scoring, cv=cv_ts
                                        , n_jobs=n_jobs)
            clf_search.fit(x, y)

            timespent = timeit(clf_class_, clf_search.best_params_, x, y)
